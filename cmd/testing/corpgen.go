...
func GenerateTestCorpus(manateePath string, corpname string, wordCount int, tagCount int, wordsPerSentence int) error {
	registryPath := fmt.Sprintf("%s/registry/%s", manateePath, corpname)
	verticalPath := fmt.Sprintf("%s/vert/%s.vert", manateePath, corpname)
	dataPath := fmt.Sprintf("%s/data/%s", manateePath, corpname)
